...
class GeneralizedRegressionWithSGD private[mllib] (
  ...
  private val gradient = new IdevGradient()
  private val updater = new SimpleUpdater()
  override val optimizer = new GradientDescent(gradient, updater)
    .setStepSize(stepSize)
    .setNumIterations(numIterations)
...
object GeneralizedRegressionWithSGD {
  ...
  def train(
      ...
      miniBatchFraction: Double,
      initialWeights: Vector): LinearRegressionModel = {
    new GeneralizedRegressionWithSGD(stepSize, numIterations, miniBatchFraction)
      .run(input, initialWeights)
  }
  ...
  def train(
      ...
      stepSize: Double,
      miniBatchFraction: Double): LinearRegressionModel = {
    new GeneralizedRegressionWithSGD(stepSize, numIterations, miniBatchFraction).run(input)
  }
